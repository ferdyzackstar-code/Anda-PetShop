...
@section('title', 'Edit Pembelian — Anda Petshop')
...
@section('content')
    ...
    <div class="card w-100 border-0 shadow-sm mb-3">
        <div class="card-body py-3 px-4 bg-warning rounded d-flex align-items-center justify-content-between"
            ...
            <h5 class="mb-0 text-white font-weight-bold">
                <i class="fas fa-shopping-cart mr-2"></i> Edit Pembelian
            </h5>
    ...
    <x-breadcrumb :items="[['label' => 'Pembelian', 'url' => route('dashboard.purchases.index')], ['label' => 'Edit']]" />
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-warning">
                <i class="fas fa-edit mr-1"></i> Edit Pesanan: <strong>{{ $purchase->purchase_number }}</strong>
            </h6>
        </div>
        <div class="card-body">
            <form id="purchaseForm" action="{{ route('dashboard.purchases.update', $purchase->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="notes" id="inputNotes" class="form-control"
                                placeholder="Catatan tambahan..." value="{{ old('notes', $purchase->notes) }}">
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            purchaseItems.forEach(item =>
                addProductRow(item.product_id, item.quantity, formatRupiah(item.price))
            );
